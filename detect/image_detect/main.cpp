...
DEFINE_string(labels, "./labels.txt", "Labels file for the yolo model.");
...
static bool parse_args(yolo::ObjectDetectionConfig& config, const std::string& path)
{
...
int main(int argc, char* argv[])
{
    ...
    std::vector<std::shared_ptr<yolo::ObjectDetection> > vec_alg;
    for (int i = 0; i < 1; i++) {
        std::shared_ptr<yolo::ObjectDetection> alg = std::shared_ptr<yolo::ObjectDetection>(new yolo::ObjectDetection());
        alg->Init(config);
    ...
    for (int i = 0; i < 1; i++) {
        ...
        std::vector<yolo::ObjectData> vec_res;
        ...
        for (size_t j = 0; j < vec_res.size(); j++) {
            yolo::ObjectData result = vec_res[j];
            LOG_INFO("[{}, {}, {}, {}, {}, {}, time cost: {}]",
                result.bbox.x,
                result.bbox.y,
                result.bbox.width,
